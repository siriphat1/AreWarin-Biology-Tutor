...
Deno.serve(async (req) => {
  ...
  if (req.method === 'GET') return json({
    ...
    version: 'v15.4.1-course-sync',
    mode: 'unified-course-sync',
  });
  ...
  try {
    ...
    catch { return json({ success:false, message:'ข้อมูลที่ส่งมาไม่ใช่ JSON ที่ถูกต้อง', version:'v15.4.1-course-sync' }, 400); }

    const fullname = clean(d.fullname);
    if (!fullname) return json({ success:false, message:'กรุณากรอกชื่อผู้สมัคร', version:'v15.4.1-course-sync' }, 400);
    ...
    // V15.4.1: create canonical course line items immediately.
    // Student Portal/Tutor OS read from enrollment_items -> os_student_course_enrollments,
    // so an application must never exist as a payment-only record without its course links.
    const uuidOrNull = (v: unknown) => {
      const x = clean(v);
      return /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(x) ? x : null;
    };
    const courseItems = Array.isArray(d.courseItems) ? d.courseItems.filter((x:any)=>x && uuidOrNull(x.courseId)) : [];
    if (courseItems.length) {
      const rawHours = clean(d.hours);
      const unlimited = clean(d.selectedMode) === 'yearly' || /รายปี/i.test(rawHours);
      const totalHours = unlimited ? null : Math.max(0, num(rawHours, num(d.hourlyCount, 0)));
      const shareMode = clean(d.cartShareMode) === 'separate' ? 'separate' : 'shared';
      const eachAmount = courseItems.length ? amount / courseItems.length : amount;
      const rows = courseItems.map((x:any) => ({
        enrollment_id: enrollment.id,
        course_id: uuidOrNull(x.courseId),
        tutor_id: uuidOrNull(x.tutorId),
        offering_id: uuidOrNull(x.offeringId),
        course_name_snapshot: clean(x.courseName) || null,
        tutor_name_snapshot: clean(x.tutorName) || null,
        package_code: clean(d.selectedMode) || clean(d.mode) || null,
        share_mode: shareMode,
        hours_allocated: totalHours,
        hours_unlimited: unlimited,
        amount_allocated: eachAmount,
        status: 'pending',
      }));
      const itemInsert = await sb.from('enrollment_items').upsert(rows, { onConflict: 'enrollment_id,course_id' });
      if (itemInsert.error) warnings.push(`COURSE_ITEMS_SYNC_FAILED:${errorText(itemInsert.error)}`);
    } else {
      warnings.push('COURSE_ITEMS_MISSING');
    }
    ...
    if (warnings.length) {
      const payloadWithWarnings = { ...sanitizedPayload, rescueWarnings: warnings, rescueVersion: 'v15.4.1-course-sync', slipPath };
    ...
    console.error('create-enrollment v15.4.1 course sync failed', { message, err, enrollmentId: enrollment?.id, slipPath, warnings });
    return json({ success:false, message, version:'v15.4.1-course-sync', warnings }, 400);
  }
});
