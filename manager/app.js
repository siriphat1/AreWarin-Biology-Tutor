...
async function loadAll() {
  const [t,c,rv,p,pr,en,pay,sp,ta,rs,as,st,sch,sr,brand,bn,cat,pp,ps] = await Promise.all([
    ...
    sb.from('policy_pages').select('*').order('page_key'),
    sb.from('policy_sections').select('*').order('page_key').order('sort_order').order('created_at')
  ]);
  for (const x of [t,c,rv,p,pr,en,pay,sp,ta,rs,as,st,sch,sr,brand,bn,cat,pp,ps]) if(x.error) console.error(x.error);
  state.tutors=t.data||[];
  ...
  state.policyPages=pp.data||[];
  state.policySections=ps.data||[];
...
function renderTutorApplications(){
  ...
  const list=all.filter(x=>{const hay=[x.application_no,x.first_name,x.last_name,x.nickname,x.phone,x.email,x.nationality,x.country_residence,x.preferred_language,...taArr(x.subjects)].join(' ').toLowerCase();return(f==='all'||x.status===f)&&(!q||hay.includes(q));});
...
// ---------- Policy & Rules CMS ----------
let activePolicyPageKey='student_enrollment';
const POLICY_PAGE_LABELS={student_enrollment:'สมัครเรียนหลัก',tutor_application:'สมัครติวเตอร์'};
function currentPolicyPage(){return state.policyPages.find(x=>x.page_key===activePolicyPageKey)||null;}
function resetPolicySectionForm(){if(!$('policySectionForm'))return;$('policySectionForm').reset();$('policySectionId').value='';$('policySectionTheme').value='blue';$('policySectionSort').value='100';$('policySectionActive').checked=true;}
function fillPolicyPageForm(){if(!$('policyPageForm'))return;const p=currentPolicyPage()||{};$('policyWelcomeKicker').value=p.welcome_kicker||'';$('policyWelcomeTitle').value=p.welcome_title||'';$('policyWelcomeHighlight').value=p.welcome_highlight||'';$('policyWelcomeDesc').value=p.welcome_description||'';$('policyTitle').value=p.policy_title||'';$('policySubtitle').value=p.policy_subtitle||'';$('policyConsentTitle').value=p.consent_title||'';$('policyConsentDesc').value=p.consent_description||'';$('policyStartLabel').value=p.start_button_label||'';$('policyWelcomeKickerEn').value=p.welcome_kicker_en||'';$('policyWelcomeTitleEn').value=p.welcome_title_en||'';$('policyWelcomeHighlightEn').value=p.welcome_highlight_en||'';$('policyWelcomeDescEn').value=p.welcome_description_en||'';$('policyTitleEn').value=p.policy_title_en||'';$('policySubtitleEn').value=p.policy_subtitle_en||'';$('policyConsentTitleEn').value=p.consent_title_en||'';$('policyConsentDescEn').value=p.consent_description_en||'';$('policyStartLabelEn').value=p.start_button_label_en||'';$('policyPageActive').checked=p.active!==false;$('policyRevisionBadge').textContent='REV. '+Number(p.revision||1);if($('policyListSubtitle'))$('policyListSubtitle').textContent=`${POLICY_PAGE_LABELS[activePolicyPageKey]||activePolicyPageKey} · จัดลำดับ เปิด/ปิด แก้ไข หรือลบได้`;}
function policySectionCard(x){const linesCount=String(x.content||'').split(/\n+/).filter(Boolean).length,enCount=String(x.content_en||'').split(/\n+/).filter(Boolean).length;return `<article class="rounded-2xl border border-slate-200 bg-white p-4 hover:border-sky-200 transition"><div class="flex items-start gap-3"><div class="w-10 h-10 rounded-xl bg-slate-50 border border-slate-100 grid place-items-center text-sky-600 shrink-0"><i class="${esc(x.icon_class||'fas fa-circle-info')}"></i></div><div class="min-w-0 flex-1"><div class="flex items-center gap-2 flex-wrap"><h3 class="text-[11px] font-semibold text-slate-800">${esc(x.title)}</h3><span class="tag">#${Number(x.sort_order||100)}</span><span class="tag">${esc(x.theme||'blue')}</span><span class="tag ${x.title_en?'!bg-indigo-50 !text-indigo-600':''}">${x.title_en?'TH + EN':'TH'}</span></div><div class="text-[8.5px] text-slate-400 mt-1">${esc(x.caption||'')} · TH ${linesCount} ข้อ${x.title_en?` · EN ${enCount} rules`:''} · ${x.active?'แสดง':'ซ่อน'}</div><div class="text-[9px] leading-relaxed text-slate-500 mt-2 line-clamp-3 whitespace-pre-line">${esc(x.content||'')}</div>${x.title_en?`<div class="text-[9px] leading-relaxed text-indigo-500 mt-2 line-clamp-2 whitespace-pre-line"><b>${esc(x.title_en)}</b> — ${esc(x.content_en||'')}</div>`:''}</div></div><div class="flex gap-2 mt-3"><button onclick="editPolicySection('${x.id}')" class="flex-1 py-2 rounded-lg bg-sky-50 text-sky-600 text-[9.5px] font-semibold">แก้ไข</button><button onclick="togglePolicySection('${x.id}',${x.active?'false':'true'})" class="px-3 py-2 rounded-lg bg-slate-50 text-slate-600 text-[9.5px] font-semibold">${x.active?'ซ่อน':'เปิด'}</button><button onclick="deletePolicySection('${x.id}')" class="px-3 py-2 rounded-lg bg-rose-50 text-rose-600 text-[9.5px] font-semibold">ลบ</button></div></article>`;}
function renderPolicyCMS(){if(!$('policyPageSelect'))return;if(!state.policyPages.some(x=>x.page_key===activePolicyPageKey))activePolicyPageKey=state.policyPages[0]?.page_key||'student_enrollment';$('policyPageSelect').value=activePolicyPageKey;fillPolicyPageForm();const rows=state.policySections.filter(x=>x.page_key===activePolicyPageKey).sort((a,b)=>Number(a.sort_order||100)-Number(b.sort_order||100));$('policySectionCount').textContent=`${rows.length} รายการ`;$('policySectionList').innerHTML=rows.map(policySectionCard).join('')||'<div class="md:col-span-2 py-10 text-center text-[10px] text-slate-400">ยังไม่มีหัวข้อนโยบาย</div>';}
async function bumpPolicyRevision(pageKey){const p=state.policyPages.find(x=>x.page_key===pageKey);if(!p)return;const{error}=await sb.from('policy_pages').update({revision:Number(p.revision||1)+1,updated_at:new Date().toISOString()}).eq('page_key',pageKey);if(error)throw error;}
if($('policyPageSelect'))$('policyPageSelect').onchange=e=>{activePolicyPageKey=e.target.value;resetPolicySectionForm();renderPolicyCMS();};
if($('policySectionReset'))$('policySectionReset').onclick=resetPolicySectionForm;
if($('policyPageForm'))$('policyPageForm').onsubmit=async e=>{e.preventDefault();try{const existing=currentPolicyPage();const row={page_key:activePolicyPageKey,audience:activePolicyPageKey==='tutor_application'?'tutor':'student',welcome_kicker:$('policyWelcomeKicker').value.trim()||null,welcome_title:$('policyWelcomeTitle').value.trim()||'AreWarin Biology',welcome_highlight:$('policyWelcomeHighlight').value.trim()||null,welcome_description:$('policyWelcomeDesc').value.trim()||null,policy_title:$('policyTitle').value.trim(),policy_subtitle:$('policySubtitle').value.trim()||null,consent_title:$('policyConsentTitle').value.trim(),consent_description:$('policyConsentDesc').value.trim()||null,start_button_label:$('policyStartLabel').value.trim()||'ยอมรับและไปต่อ',welcome_kicker_en:$('policyWelcomeKickerEn').value.trim()||null,welcome_title_en:$('policyWelcomeTitleEn').value.trim()||null,welcome_highlight_en:$('policyWelcomeHighlightEn').value.trim()||null,welcome_description_en:$('policyWelcomeDescEn').value.trim()||null,policy_title_en:$('policyTitleEn').value.trim()||null,policy_subtitle_en:$('policySubtitleEn').value.trim()||null,consent_title_en:$('policyConsentTitleEn').value.trim()||null,consent_description_en:$('policyConsentDescEn').value.trim()||null,start_button_label_en:$('policyStartLabelEn').value.trim()||null,revision:Number(existing?.revision||0)+1,active:$('policyPageActive').checked,updated_at:new Date().toISOString()};const{error}=await sb.from('policy_pages').upsert(row,{onConflict:'page_key'});if(error)throw error;await loadAll();notify('success','บันทึกนโยบายแล้ว',`อัปเดตเป็น Revision ${row.revision} · รองรับ TH/EN`);}catch(err){notify('error','บันทึกนโยบายไม่สำเร็จ',err.message)}};
if($('policySectionForm'))$('policySectionForm').onsubmit=async e=>{e.preventDefault();try{const row={page_key:activePolicyPageKey,title:$('policySectionTitle').value.trim(),caption:$('policySectionCaption').value.trim()||null,title_en:$('policySectionTitleEn').value.trim()||null,caption_en:$('policySectionCaptionEn').value.trim()||null,icon_class:$('policySectionIcon').value.trim()||'fas fa-circle-info',theme:$('policySectionTheme').value,content:$('policySectionContent').value.trim(),content_en:$('policySectionContentEn').value.trim()||null,sort_order:Number($('policySectionSort').value||100),active:$('policySectionActive').checked,updated_at:new Date().toISOString()};let q=$('policySectionId').value?sb.from('policy_sections').update(row).eq('id',$('policySectionId').value):sb.from('policy_sections').insert(row);const{error}=await q;if(error)throw error;await bumpPolicyRevision(activePolicyPageKey);resetPolicySectionForm();await loadAll();notify('success','บันทึกหัวข้อกติกาแล้ว');}catch(err){notify('error','บันทึกหัวข้อไม่สำเร็จ',err.message)}};
window.editPolicySection=id=>{const x=state.policySections.find(v=>v.id===id);if(!x)return;activePolicyPageKey=x.page_key;$('policyPageSelect').value=x.page_key;$('policySectionId').value=x.id;$('policySectionTitle').value=x.title||'';$('policySectionCaption').value=x.caption||'';$('policySectionTitleEn').value=x.title_en||'';$('policySectionCaptionEn').value=x.caption_en||'';$('policySectionIcon').value=x.icon_class||'';$('policySectionTheme').value=x.theme||'blue';$('policySectionContent').value=x.content||'';$('policySectionContentEn').value=x.content_en||'';$('policySectionSort').value=x.sort_order??100;$('policySectionActive').checked=x.active!==false;$('policySectionForm').scrollIntoView({behavior:'smooth',block:'center'});};
window.togglePolicySection=async(id,active)=>{try{const x=state.policySections.find(v=>v.id===id);if(!x)return;const{error}=await sb.from('policy_sections').update({active,updated_at:new Date().toISOString()}).eq('id',id);if(error)throw error;await bumpPolicyRevision(x.page_key);await loadAll();}catch(err){notify('error','เปลี่ยนสถานะไม่สำเร็จ',err.message)}};
window.deletePolicySection=async id=>{const x=state.policySections.find(v=>v.id===id);if(!x)return;const r=await Swal.fire({icon:'warning',title:'ลบหัวข้อนโยบายนี้?',text:x.title,showCancelButton:true,confirmButtonText:'ลบ',cancelButtonText:'ยกเลิก',confirmButtonColor:'#e11d48'});if(!r.isConfirmed)return;try{const{error}=await sb.from('policy_sections').delete().eq('id',id);if(error)throw error;await bumpPolicyRevision(x.page_key);if($('policySectionId').value===id)resetPolicySectionForm();await loadAll();notify('success','ลบหัวข้อนโยบายแล้ว');}catch(err){notify('error','ลบไม่สำเร็จ',err.message)}};
